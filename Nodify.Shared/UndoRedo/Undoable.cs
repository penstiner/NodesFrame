...
namespace Nodify.UndoRedo
{
    ...
    public abstract class Undoable : ObservableObject
    {
        ...
        {
            if (memberAccess is LambdaExpression lambda)
                return GetPropertyName(lambda.Body);
            if (memberAccess is MemberExpression mbr)
                return mbr.Member.Name;
            if (memberAccess is UnaryExpression unary)
                return GetPropertyName(unary.Operand);

            throw new Exception(string.Format("Member name could not be extracted from {0}.", memberAccess));
        }
